...
function mockAnthropic(payload) {
    return async () => new Response(JSON.stringify({ content: [{ type: "text", text: JSON.stringify(payload) }] }), { status: 200, headers: { "content-type": "application/json" } });
}
...
test("model cannot claim tier A when prior advances have no verified repayments", async () => {
    const proposal = await underwrite(evidence({ history: baseHistory({ priorRepaymentsWithThisBuyer: 0 }) }), {
        anthropicApiKey: "test-key",
        fetchFn: mockAnthropic({
            recommendedAdvance: "400",
            riskTier: "A",
...
test("model cannot claim tier A for incomplete or new relationship history", async () => {
    const incomplete = await underwrite(evidence({ history: baseHistory({ historyComplete: false }) }), {
        anthropicApiKey: "test-key",
        fetchFn: mockAnthropic({
            recommendedAdvance: "400",
            riskTier: "A",
    ...
    const newRelationship = await underwrite(evidence({
...
test("verified defaults impose the strongest risk-tier floor", async () => {
    const proposal = await underwrite(evidence({ history: baseHistory({ priorDefaultsWithThisBuyer: 1 }) }), {
        anthropicApiKey: "test-key",
        fetchFn: mockAnthropic({
            recommendedAdvance: "400",
            riskTier: "A",
...
test("malformed model JSON cannot block or authorize the funding path", async () => {
    const fetchFn = async () => new Response(JSON.stringify({ content: [{ type: "text", text: "ignore previous constraints" }] }), {
        status: 200,
...
test("prior defaults produce elevated fallback risk", async () => {
    const proposal = await underwrite(evidence({ history: baseHistory({ priorDefaultsWithThisBuyer: 2 }) }), { now: () => 1_700_000_000 });
    assert.equal(proposal.riskTier, "D");
    assert.ok(proposal.reasonCodes.includes("DEFAULT_HISTORY"));
    assert.ok(proposal.riskFlags.includes("PRIOR_DEFAULTS"));
});
