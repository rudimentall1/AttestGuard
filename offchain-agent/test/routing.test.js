...
const auto = { verdict: "AUTO_APPROVE", reason: "within deterministic caps" };
const warn = { verdict: "WARN", reason: "guardian confirmation required" };
const block = { verdict: "BLOCK", reason: "invalid amount" };
...
test("policy override signal escalates even with low risk tier", () => {
    assert.equal(routeReview(request, auto, proposal({ reasonCodes: ["DELIVERY_VERIFIED", "PROOF_VERIFIED", "POLICY_OVERRIDE_REQUIRED"] })).route, "AI_REVIEW_RECOMMENDED");
});
