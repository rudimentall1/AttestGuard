...
test("blocks an advance that requests more than the invoice is worth", () => {
    const decision = evaluateAdvancePolicy(baseRequest({ requestedAdvanceAmount: 1500n }), baseHistory());
...
test("warns when amount exceeds the supplier's auto-approve cap", () => {
    const decision = evaluateAdvancePolicy(baseRequest({ requestedAdvanceAmount: 600n }), baseHistory({ autoApproveCap: 500n }));
...
test("warns when funding would breach the supplier's daily cap even under the per-advance cap", () => {
    const decision = evaluateAdvancePolicy(baseRequest({ requestedAdvanceAmount: 400n }), baseHistory({ autoApproveCap: 1000n, fundedToday: 1800n, perSupplierDailyCap: 2000n }));
    assert.equal(decision.verdict, "WARN");
...
test("blocks a buyer with a prior default because the evidence is not enforced on-chain", () => {
    const decision = evaluateAdvancePolicy(baseRequest({ requestedAdvanceAmount: 50n }), baseHistory({ priorDefaultsWithThisBuyer: 1 }));
    assert.equal(decision.verdict, "BLOCK");
...
test("a defaulted buyer takes priority over an otherwise-clean cap check", () => {
    const decision = evaluateAdvancePolicy(baseRequest({ requestedAdvanceAmount: 5000n, invoiceAmount: 20000n }), baseHistory({ autoApproveCap: 10000n, perSupplierDailyCap: 10000n, priorDefaultsWithThisBuyer: 2 }));
    assert.equal(decision.verdict, "BLOCK");
